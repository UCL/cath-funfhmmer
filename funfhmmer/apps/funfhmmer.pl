...
#    [ 'Screen', min_level => 'warning', stderr => 1, newline => 1 ],
    [ 'File', min_level => 'debug', filename => "$funfam_dir/$superfamily.$treename.LOG" ],
  ],
  callbacks => sub { 
    my %p = @_;
    my @lines = split (/\n/, $p{message});
...
$LOG->info( "# Processing $superfamily $treename tree" );
...
#####
system("tar -zcf $dir/funfam_alignments/$superfamily.analysis_data.tar.gz -C $dir/funfam_alignments ./analysis_data/");
