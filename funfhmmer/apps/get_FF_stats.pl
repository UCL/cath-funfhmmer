...
my $DIR= path("/cath/homes2/ucbtdas/Working/eclipse_workspace/FunFHMMer_code/github_resources/scripts/FFer_standalone_TL_bchuckle");
my $DATADIR= $DIR->child("data");
my $APPSDIR= $DIR->child("apps");
my $RESULTSDIR= $DIR->child("results");
my $ANNODIR = $DIR->path("anno");
unless($ANNODIR->exists){
	$ANNODIR->mkpath;
}
...
my $STARTINGCLUSTERDATA= path("/cath/people2/ucbctnl/GeMMA/v4_0_0/starting_clusters");

# connect to the database
my $db = Cath::Schema::Biomap->connect_by_version("4.1");
...
foreach my $SF (@superfamilies){
	...
	foreach my $sftree (@sftreelines){
		chomp($sftree);
		my ($sf, $TREE) = split(" ", $sftree);
		if($sf eq $SF){
			if(-e "$RESULTSDIR/$SF.$TREE/"){
			print "--- $TREE\n";
			...
			foreach my $aln (glob("$RESULTSDIR/$SF.$TREE/*.aln")) {
				...
				my $aln_dops = Funfhmmer::Scorecons::assign_dops_score($alnname,"$RESULTSDIR/$SF.$TREE");
				if($aln_dops >= 70){
					$highdops_ff++;
